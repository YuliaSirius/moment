'use strict';
// localStorage.removeItem('showPreloader');
if (localStorage.getItem('showPreloader') == 'false') {
} else {
  let imagesHome = [
    './img/manager/manager_main_bg.png',
    './img/home/manager1x.png',
    './img/home/manager2x.png',
    './img/home/manager3x.png',
    './img/home/manager1x.webp',
    './img/home/manager2x.webp',
    './img/home/manager3x.webp',
    './img/home/woman1x.png',
    './img/home/woman2x.png',
    './img/home/woman3x.png',
    './img/home/woman1x.webp',
    './img/home/woman2x.webp',
    './img/home/woman3x.webp',
    './img/home/engineer1x.png',
    './img/home/engineer2x.png',
    './img/home/engineer3x.png',
    './img/home/engineer1x.webp',
    './img/home/engineer2x.webp',
    './img/home/engineer3x.webp',
    './img/home/menu_image1x.png',
    './img/home/menu_image2x.png',
    './img/home/menu_image3x.png',
    './img/home/menu_image1x.webp',
    './img/home/menu_image2x.webp',
    './img/home/menu_image3x.webp',
    './img/engineer/engineer_main_bg.png',
    './img/engineer/engineer_main_bg2x.png',
    './img/engineer/engineer_main_bg3x.png',
    './img/engineer/engineer_main_bg.webp',
    './img/engineer/engineer_main_bg2x.webp',
    './img/engineer/engineer_main_bg3x.webp',
    './img/engineer/engineer_poster.jpg',
    './img/engineer/engineer_poster.webp',
    './img/engineer/rotate.jpg',
    './img/engineer/rotate2x.jpg',
    './img/engineer/rotate3x.jpg',
    './img/engineer/rotate.webp',
    './img/engineer/rotate2x.webp',
    './img/engineer/rotate3x.webp',
    './img/engineer/load.jpg',
    './img/engineer/load2x.jpg',
    './img/engineer/load3x.jpg',
    './img/engineer/load.webp',
    './img/engineer/load2x.webp',
    './img/engineer/load3x.webp',
    './img/engineer/trajectory.png',
    './img/engineer/trajectory.webp',
    './img/employee/employee_main_bg.png',
    './img/employee/employee_main_bg2x.png',
    './img/employee/employee_main_bg3x.png',
    './img/employee/employee_main_bg.webp',
    './img/employee/employee_main_bg2x.webp',
    './img/employee/employee_main_bg3x.webp',
    './img/employee/rotate.jpg',
    './img/employee/rotate2x.jpg',
    './img/employee/rotate3x.jpg',
    './img/employee/rotate.webp',
    './img/employee/rotate2x.webp',
    './img/employee/rotate3x.webp',
    './img/employee/load.jpg',
    './img/employee/load2x.jpg',
    './img/employee/load3x.jpg',
    './img/employee/load.webp',
    './img/employee/load2x.webp',
    './img/employee/load3x.webp',
    './img/employee/trajectory.png',
    './img/employee/trajectory.webp',
    './img/manager/manager_main_bg.png',
    './img/manager/manager_main_bg2x.png',
    './img/manager/manager_main_bg3x.png',
    './img/manager/manager_main_bg.webp',
    './img/manager/manager_main_bg2x.webp',
    './img/manager/manager_main_bg3x.webp',
    './img/manager/manager_poster.jpg',
    './img/manager/manager_poster.webp',
    './img/manager/rotate.jpg',
    './img/manager/rotate2x.jpg',
    './img/manager/rotate3x.jpg',
    './img/manager/rotate.webp',
    './img/manager/rotate2x.webp',
    './img/manager/rotate3x.webp',
    './img/manager/load.jpg',
    './img/manager/load2x.jpg',
    './img/manager/load3x.jpg',
    './img/manager/load.webp',
    './img/manager/load2x.webp',
    './img/manager/load3x.webp',
    './img/manager/trajectory.png',
    './img/manager/trajectory.webp',
    './img/connections/moment.png',
    './img/connections/moment2x.png',
    './img/connections/moment3x.png',
    './img/connections/moment.webp',
    './img/connections/moment2x.webp',
    './img/connections/moment3x.webp',
    './img/connections/moment_gt.png',
    './img/connections/moment_gt2x.png',
    './img/connections/moment_gt3x.png',
    './img/connections/moment_gt.webp',
    './img/connections/moment_gt2x.webp',
    './img/connections/moment_gt3x.webp',
    './img/connections/moment_fl.png',
    './img/connections/moment_fl2x.png',
    './img/connections/moment_fl3x.png',
    './img/connections/moment_fl.webp',
    './img/connections/moment_fl2x.webp',
    './img/connections/moment_fl3x.webp',
    './img/connections/moment_sfl.png',
    './img/connections/moment_sfl2x.png',
    './img/connections/moment_sfl3x.png',
    './img/connections/moment_sfl.webp',
    './img/connections/moment_sfl2x.webp',
    './img/connections/moment_sfl3x.webp',
    './img/moment/moment_content_img.png',
    './img/moment/moment_content_img2x.png',
    './img/moment/moment_content_img3x.png',
    './img/moment/moment_content_img.webp',
    './img/moment/moment_content_img2x.webp',
    './img/moment/moment_content_img3x.webp',
    './img/moment/momentum_bg.png',
    './img/moment/momentum_bg2x.png',
    './img/moment/momentum_bg3x.png',
    './img/moment/momentum_bg.webp',
    './img/moment/momentum_bg2x.webp',
    './img/moment/momentum_bg3x.webp',
    './img/gt/momentumGT_bg.png',
    './img/gt/momentumGT_bg2x.png',
    './img/gt/momentumGT_bg3x.png',
    './img/gt/momentumGT_bg.webp',
    './img/gt/momentumGT_bg2x.webp',
    './img/gt/momentumGT_bg3x.webp',
    './img/gt/img_gt.png',
    './img/gt/img_gt2x.png',
    './img/gt/img_gt3x.png',
    './img/gt/img_gt.webp',
    './img/gt/img_gt2x.webp',
    './img/gt/img_gt3x.webp',
    './img/fl/fl_image.png',
    './img/fl/fl_image2x.png',
    './img/fl/fl_image3x.png',
    './img/fl/fl_image.webp',
    './img/fl/fl_image2x.webp',
    './img/fl/fl_image3x.webp',
    './img/fl/moment_content_img_fl.png',
    './img/fl/moment_content_img_fl2x.png',
    './img/fl/moment_content_img_fl3x.png',
    './img/fl/moment_content_img_fl.webp',
    './img/fl/moment_content_img_fl2x.webp',
    './img/fl/moment_content_img_fl3x.webp',
    './img/sfl/sfl_image.png',
    './img/sfl/sfl_image2x.png',
    './img/sfl/sfl_image3x.png',
    './img/sfl/sfl_image.webp',
    './img/sfl/sfl_image2x.webp',
    './img/sfl/sfl_image3x.webp',
    './img/sfl/moment_content_img_sfl.png',
    './img/sfl/moment_content_img_sfl2x.png',
    './img/sfl/moment_content_img_sfl3x.png',
    './img/sfl/moment_content_img_sfl.webp',
    './img/sfl/moment_content_img_sfl2x.webp',
    './img/sfl/moment_content_img_sfl3x.webp',
  ];

  function classElem(v) {
    return document.querySelector(v);
  }
  let preloader = document.createElement('div');
  preloader.className = 'preload';
  preloader.innerHTML = `<svg
  width="58"
  height="58"
  viewBox="0 0 58 58"
  fill="none"
  xmlns="http://www.w3.org/2000/svg"
  xmlns:xlink="http://www.w3.org/1999/xlink"      >
  <rect width="58" height="58" fill="url(#pattern0)" />
  <defs>
    <pattern
      id="pattern0"
      patternContentUnits="objectBoundingBox"
      width="1"
      height="1">
      <use xlink:href="#image0" transform="scale(0.00625)" />
    </pattern>
    <image
      id="image0"
      width="160"
      height="160"
      xlink:href="data:image/png;base64,"
    />
  </defs>
  </svg>
  <div class="progstat"></div>
  <div class="progress"></div>`;
  document.body.append(preloader);

  window.addEventListener('load', loadbar, false);
  function loadbar() {
    let preload = classElem('.preload'),
      prog = classElem('.progress'),
      c = 0,
      tot = imagesHome.length;

    function doneLoading() {
      preload.classList.add('loaded_hiding');
      window.setTimeout(function () {
        preload.classList.add('loaded');
        preload.classList.remove('loaded_hiding');
        localStorage.setItem('showPreloader', 'false');
      }, 500);
    }
    function imgLoaded() {
      c += 1;
      let perc = (((100 / tot) * c) << 0) + '%';
      prog.style.width = perc;
      if (c === tot) return doneLoading();
    }

    function preloadImages() {
      for (let i = 0; i < arguments.length; i++) {
        let tImg = new Image();
        tImg.onload = imgLoaded;
        tImg.onerror = imgLoaded;
        tImg.src = arguments[i];
        console.log(tImg);
      }
    }

    for (let i = 0; i < imagesHome.length; i++) {
      preloadImages(imagesHome[i]);
    }
  }
}
